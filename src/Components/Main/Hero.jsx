...
const Hero = () => {
  return (
    <div 
    className='h-screen px-4 py-2 text-center bg-gradient-to-r from-slate-800 to-bg-primary flex flex-col gap-2 items-center pt-30'>
      <motion.h1 
      ...
      className='mt-8 max-w-lg w-sm text-slate-400 md:w-md lg:w-lg text-lg'>I build exceptional digital experiences with modern technologies. Focused on creating clean, user-friendly, and innovative solutions.</motion.p>
      <motion.div 
      initial={{translateY: 25, opacity: 0}}
      whileInView={{translateY: 0, opacity: 1}}
      transition={{duration: 0.8, ease: ['easeInOut']}}
      className='mt-8 flex gap-4 md:gap-8'>
        <a href="" className="px-6 py-3 rounded-lg bg-secondary hover:bg-indigo-900 transition-all duration-300 ease-in-out">Get in Touch</a>
        <a href="" className="px-6 py-3 rounded-lg border-2 text-secondary border-secondary hover:bg-slate-300/5 transition-all duration-300 ease-in-out">View My Work</a>
      </motion.div>
      <motion.div 
      initial={{translateY: 25, opacity: 0}}
      whileInView={{translateY: 0, opacity: 1}}
      ...
      <a href="" className='mt-8 md:mt-16 animate-bounce text-slate-500 transition-colors duration-300 ease-in-out hover:text-secondary'>
        <ArrowDown size={25} />
      </a>
    </div>
  )
